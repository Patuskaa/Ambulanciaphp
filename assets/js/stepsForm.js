...
;( function( window ) {
	...
	stepsForm.prototype._init = function() {
		...
		//passa para a anterior pergunta
		this.ctrlPrev = this.el.querySelector ('button.prev');

		// progress bar
		this.progress = this.el.querySelector( 'div.progress' );

		this.ambulance = document.querySelector('.ambulanceimg' );
		...
		for (var i = this.questions.length - 1; i >= 0; i--) {
			var formElement = this.questions[i].querySelector( 'input, textarea, select, checkbox' );
	...
	stepsForm.prototype._initEvents = function() {
		var self = this,
			// first input
			firstElInput = this.questions[ this.current ].querySelector( 'input, textarea, select, checkbox' ),
			// focus
	...
	stepsForm.prototype._nextQuestion = function() {
		...
		if ( this.supportsHTML5Forms ) {
			var input = this.questions[ this.current ].querySelector( 'input, textarea, select, checkbox' );
		...
		var self = this,
			onEndTransitionFn = function( ev ) {
				...
				if( self.isFilled ) {
					...
					nextQuestion.querySelector( 'input, textarea, select, checkbox' ).focus();
				}
			};
	...
	stepsForm.prototype._ambulance = function(){
		var currentProgress = this.current * ( 100 / this.questionsCount );
		this.ambulance.style.left = currentProgress + '%';
	}
	...
	stepsForm.prototype._validate = function() {
		...
		var input = this.questions[ this.current ].querySelector( 'input, textarea, select, checkbox' ).value;
		if( input === '' ) {
	...
	stepsForm.prototype._showError = function( err ) {
		...
		switch( err ) {
			case 'EMPTYSTR' :
				message = 'Preencha o campo antes de prosseguir';
				break;
			case 'INVALIDEMAIL' :
